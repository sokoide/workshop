...
複数のサーバーにトラフィックを分散する装置またはソフトウェア。可用性とスケーラビリティを向上させる。ロードバランサー自体が単一障害点（SPOF）にならないよう、冗長化も必要になる。
...
直近の時間区間でリクエスト数を測る方式。スライディングログは個々のリクエストを記録し、重み付きカウンターは隣接する窓の件数から近似する。
