...
クライアントからのリクエストを Envoy が受け取り、バックエンドサービスへ転送します。Envoy 上で意図的な遅延やエラー（Fault Injection）を発生させ、挙動を確認します。
...
レスポンスヘッダーに `server: envoy` が含まれていることを確認してください。

### STEP 2: タイムアウトの動作確認

バックエンドサービスが遅延した場合の挙動を確認します。設定ファイルで `timeout: 2s` と設定されています。
バックエンドに 3 秒のスリープを要求します（実習用アプリの機能）。

```bash
curl -v http://localhost:10000/sleep/3
```

**結果**: Envoy が 2 秒で接続を切り、`504 Gateway Timeout` を返すことを確認します。アプリケーションが応答を待ち続けてリソースを枯渇させるのを防ぎます。
...
ここで `retry` や `timeout` の発生回数がメトリクスとして記録されていることを確認します。
...
ビジネスロジック（Usecase/Domain）は、ネットワークの不安定さやリトライ制御といった「詳細」から守られ、純粋なロジックに集中できます。インフラストラクチャの責務をコードからサイドカーへ委譲する究極の形です。
